import"./modulepreload-polyfill.js";import{n as I,d as L}from"./import.js";import{s as i}from"./db_conn.js";import{i as T,c as M}from"./auth.js";fetch("navbar.html").then(t=>t.text()).then(t=>{document.getElementById("navbar").innerHTML=t;const e=document.createElement("script");e.type="module",e.src=I,document.body.appendChild(e)}).catch(t=>console.error("Error loading navbar:",t));T();(async()=>await M())();function x(){const t=document.getElementById("bakulForm");t&&(t.reset(),t.dataset.bakulId="",document.getElementById("bakulModalLabel").textContent="Tambah Bakul",document.getElementById("submitButton").textContent="Tambah")}function d(t,e="success"){const a=document.getElementById("toastContainer"),n=document.createElement("div");n.classList.add("toast"),n.setAttribute("role","alert"),n.setAttribute("aria-live","assertive"),n.setAttribute("aria-atomic","true"),n.classList.add(e==="success"?"bg-success":"bg-danger"),n.classList.add("text-white");const o=document.createElement("div");o.classList.add("toast-body"),o.textContent=t,n.appendChild(o),a.appendChild(n),new bootstrap.Toast(n,{autohide:!0,delay:3e3}).show(),n.addEventListener("hidden.bs.toast",()=>{n.remove()})}async function _(){const t=document.getElementById("kota");if(!t){console.warn("Kota dropdown not found in the DOM.");return}try{const{data:e,error:a}=await i.from("kota").select("id, kota");if(a)throw a;t.innerHTML='<option value="" disabled selected>Pilih Kota</option>',e.forEach(n=>{const o=document.createElement("option");o.value=n.id,o.text=n.kota,t.appendChild(o)})}catch(e){console.error("Error fetching cities:",e.message),d("Failed to load cities. Please try again.","error")}}async function C(){const t=document.getElementById("nama").value,e=document.getElementById("no_hp").value,a=document.getElementById("kota").value,n=document.getElementById("bakulForm").dataset.bakulId;try{if(n){const{data:s,error:l}=await i.from("bakul").update({nama:t,no_hp:e,id_kota:a}).eq("id",n);if(l)throw l;d("Data bakul berhasil diperbarui!","success")}else{const{data:s,error:l}=await i.from("bakul").insert([{nama:t,no_hp:e,id_kota:a}]);if(l)throw l;d("Data bakul berhasil ditambahkan!","success")}bootstrap.Modal.getInstance(document.getElementById("bakulModal")).hide(),await k()}catch(o){console.error("Error saving supplier:",o.message),d("Gagal menyimpan data bakul, silahkan coba lagi.","error")}}async function F(t){try{const{data:e,error:a}=await i.from("bakul").select("*").eq("id",t).single();if(a)throw a;document.getElementById("nama").value=e.nama,document.getElementById("no_hp").value=e.no_hp,document.getElementById("kota").value=e.id_kota,document.getElementById("bakulModalLabel").textContent="Edit Bakul",document.getElementById("submitButton").textContent="Edit",document.getElementById("bakulForm").dataset.bakulId=t,new bootstrap.Modal(document.getElementById("bakulModal")).show()}catch(e){console.error("Error fetching supplier:",e.message),d("Gagal mengambil data bakul, silahkan coba lagi.","error")}}async function H(t){if(confirm("Apakah Anda yakin ingin menghapus data bakul ini?"))try{const{data:e,error:a}=await i.from("bakul").delete().eq("id",t);if(a)throw a;await k(),d("Data bakul berhasil dihapus!","success")}catch(e){console.error("Error deleting supplier:",e.message),d("Gagal menghapus data bakul, silahkan coba lagi.","error")}}async function k(){const t=document.getElementById("bakulTableBody");if(t)try{t.innerHTML=`
            <tr>
        ...
        <div class="card-footer bg-light">
            ...
            <tr>
                ...
                no_hp,
                kota:id_kota(kota)
            `).order("id",{ascending:!0});r.type==="kota"&&r.kota&&(a=a.eq("id_kota",r.kota));const n=a;t&&n.or(`nama.ilike.%${t}%,no_hp.ilike.%${t}%`);const{data:o,error:s}=await n;let l=[];if(t&&!(r.type==="kota"&&r.kota)){const{data:c,error:u}=await i.from("kota").select("id, kota").ilike("kota",`%${t}%`);if(!u&&(c==null?void 0:c.length)>0){const m=c.map(w=>w.id),{data:b,error:v}=await a.in("id_kota",m);v||(l=b||[])}}let p;t?p=[...o||[],...l].reduce((u,m)=>(u.some(b=>b.id===m.id)||u.push(m),u),[]):p=o||[],B(p)}catch(e){console.error("Error searching bakuls:",e),d("Gagal melakukan pencarian bakul","error"),document.getElementById("bakulTableBody").innerHTML=`
            <tr>
                <td colspan="5" class="text-center text-danger">
                    Gagal memuat data bakul
                </td>
        ...
        `,e.appendChild(n)})}function y(){const t=document.getElementById("searchInput"),e=document.getElementById("searchIcon");t.value.trim()?e.innerHTML=`
            ...
            </svg>`}function G(){document.getElementById("searchInput").value="",y(),E()}document.addEventListener("DOMContentLoaded",async()=>{await L(),await _(),await k(),$();const t=document.getElementById("bakulForm");t&&t.addEventListener("submit",async o=>{o.preventDefault(),await C()});const e=document.getElementById("bakulModal");e&&e.addEventListener("hidden.bs.modal",()=>{x()});const a=document.getElementById("searchInput"),n=document.getElementById("searchButton");n&&a&&(n.addEventListener("click",()=>{a.value.trim()&&document.getElementById("searchIcon").innerHTML.includes("bi-x")?G():h()}),a.addEventListener("keyup",o=>{y(),o.key==="Enter"&&h()}),a.addEventListener("input",y))});window.editBakul=F;window.deleteBakul=H;

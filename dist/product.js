...
import"./modulepreload-polyfill.js";import{_ as $,s as d}from"./db_conn.js";import{i as V,c as A}from"./auth.js";import{d as G}from"./import.js";fetch("navbar.html").then(e=>e.text()).then(async e=>{document.getElementById("navbar").innerHTML=e,(await $(()=>import("./navbar2.js"),__vite__mapDeps([0,1,2]))).setupLogout()}).catch(e=>console.error("Error loading navbar:",e));V();(async()=>await A())();let T,B=null,o={type:null,category:null,subcategory:null,supplier:null,variant:null};const j=new Map([[.25,"1/4 lusin"],[.5,"1/2 lusin"],[1,"1 lusin"]]);function R(){var e,t;document.querySelectorAll('input[name="filterGroup"]').forEach(a=>{a.addEventListener("click",function(r){if(this.checked&&o.type===this.value){this.checked=!1,I();return}o.type=this.value,K()})}),(e=document.getElementById("applyFilter"))==null||e.addEventListener("click",C),(t=document.getElementById("resetFilter"))==null||t.addEventListener("click",I)}async function K(){var t,a;const e=document.getElementById("filterOptionsContainer");switch(e.innerHTML="",document.getElementById("filterCard").classList.remove("d-none"),o.type){case"kategori":e.innerHTML=`
            ...
            `).order("id",{ascending:!0});switch(o.type){case"kategori":o.category&&(o.subcategory?t=t.eq("id_subkategori",o.subcategory):t=t.eq("subkategori.kategori.id",o.category));break;case"supplier":o.supplier&&(t=t.eq("id_supplier",o.supplier));break}const{data:a,error:r}=await t;if(r)throw r;const{data:n,error:s}=await d.from("produk_varian").select("*").order("varian",{ascending:!0});if(s)throw s;const u=n.reduce((h,p)=>(h[p.id_produk]||(h[p.id_produk]=[]),h[p.id_produk].push(p),h),{});let k=a;o.type==="varian"&&o.variant&&(k=a.filter(h=>{const p=u[h.id]||[],i=p.length>1||p.length===1&&p[0].varian.toLowerCase()!=="standar";return o.variant==="has"?i:o.variant==="none"?!i:!0})),H(k,u)}catch(t){console.error("Error filtering products:",t),m("Gagal memfilter produk","error"),e.innerHTML=`
            <tr>
                <td colspan="7" class="text-center text-danger">
                    Gagal memuat data produk
                </td>
            </tr>`}}async function N(e){try{const{data:t,error:a}=await d.from("subkategori").select("id, subkategori").eq("id_kategori",e).order("subkategori",{ascending:!0});if(a)throw a;const r=document.getElementById("filterSubcategory");r.innerHTML='<option value="">Pilih Subkategori</option>',t.forEach(n=>{const s=document.createElement("option");s.value=n.id,s.textContent=n.subkategori,r.appendChild(s)})}catch(t){console.error("Error loading subcategories:",t),m("Gagal memuat daftar subkategori","error")}}function C(){switch(o.category=null,o.subcategory=null,o.supplier=null,o.variant=null,o.type){case"kategori":o.category=document.getElementById("filterCategory").value,o.subcategory=document.getElementById("filterSubcategory").value;break;case"supplier":o.supplier=document.getElementById("dropdownSupplier").value;break;case"varian":const e=document.querySelector('input[name="variantOption"]:checked');o.variant=(e==null?void 0:e.value)||null;break}M()}function I(){document.querySelectorAll('input[name="filterGroup"]').forEach(e=>{e.checked=!1}),o={type:null,category:null,subcategory:null,supplier:null,variant:null},document.getElementById("filterCard").classList.add("d-none"),E()}async function w(){const e=document.getElementById("searchInput").value.trim().toLowerCase();try{const t=document.getElementById("productTableBody");t.innerHTML=`
            <tr>
                <td colspan="7" class="text-center">
                    <div class="spinner-border spinner-border-sm" role="status">
                        <span class="visually-hidden">Loading...</span>
            ...
            `).not("id_supplier","is",null).order("id",{ascending:!0}),u=(n==null?void 0:n.filter(l=>{var c,b,v,f;return((b=(c=l.supplier)==null?void 0:c.perusahaan)==null?void 0:b.toLowerCase().includes(e))||((f=(v=l.supplier)==null?void 0:v.cp)==null?void 0:f.toLowerCase().includes(e))}))||[],{data:k,error:h}=await d.from("produk").select(`
                ...
                subkategori!inner(id, subkategori, kategori!inner(id, kategori)),
                supplier:supplier (perusahaan, cp)
            `).not("id_subkategori","is",null).order("id",{ascending:!0}),p=(k==null?void 0:k.filter(l=>{var c,b,v,f,P;return((b=(c=l.subkategori)==null?void 0:c.subkategori)==null?void 0:b.toLowerCase().includes(e))||((P=(f=(v=l.subkategori)==null?void 0:v.kategori)==null?void 0:f.kategori)==null?void 0:P.toLowerCase().includes(e))}))||[];let y=[...a||[],...u,...p].reduce((l,c)=>(l.some(b=>b.id===c.id)||l.push(c),l),[]);if(o.type)switch(o.type){case"kategori":o.category&&(y=y.filter(l=>{var c,b;return o.subcategory?l.id_subkategori===o.subcategory:((b=(c=l.subkategori)==null?void 0:c.kategori)==null?void 0:b.id)===o.category}));break;case"supplier":o.supplier&&(y=y.filter(l=>l.id_supplier===o.supplier));break;case"varian":break}const{data:F,error:q}=await d.from("produk_varian").select("*").order("varian",{ascending:!0});if(q)throw q;const S=F.reduce((l,c)=>(l[c.id_produk]||(l[c.id_produk]=[]),l[c.id_produk].push(c),l),{});o.type==="varian"&&o.variant&&(y=y.filter(l=>{const c=S[l.id]||[],b=c.length>1||c.length===1&&c[0].varian.toLowerCase()!=="standar";return o.variant==="has"?b:!b})),H(y,S)}catch(t){console.error("Error searching products:",t),m("Gagal melakukan pencarian","error"),document.getElementById("productTableBody").innerHTML=`
            <tr>
                <td colspan="7" class="text-center text-danger">
                    Gagal memuat data produk
                </td>
        ...
        `}function U(){document.getElementById("searchInput").value="",L(),M()}async function x(e="add",t=null){B=t,document.getElementById("productModal"),document.getElementById("productForm").reset(),document.getElementById("variantsTableBody").innerHTML="",document.getElementById("productModalLabel").textContent=e==="add"?"Tambah Produk":"Edit Produk",document.getElementById("saveProductBtn").textContent=e==="add"?"Simpan":"Update",document.getElementById("saveProductBtn").textContent=e==="add"?"Simpan":"Update",e==="edit"&&t?await z(t):_(),T.show()}async function z(e){try{const{data:t,error:a}=await d.from("produk").select(`
                id, 
                nama, 
                id_subkategori, 
                id_supplier,
                qty_minimum
            `).eq("id",e).single();if(a)throw a;const{data:r,error:n}=await d.from("produk_varian").select("*").eq("id_produk",e);if(n)throw n;document.getElementById("nama_produk").value=t.nama,document.getElementById("subkategori").value=t.id_subkategori,document.getElementById("supplier").value=t.id_supplier,document.getElementById("qty_minimum").value=t.qty_minimum;const s=document.getElementById("variantsTableBody");s.innerHTML="",r&&r.length>0?r.forEach(u=>{_({id_varian:u.id,varian:u.varian,harga_standar:u.harga_standar,jumlah_stok:u.jumlah_stok,stok_reservasi:u.stok_reservasi})}):_()}catch(t){console.error("Error loading product:",t),m("Gagal memuat data produk","error")}}async function _(e=null){const t=document.getElementById("variantsTableBody"),a=document.createElement("tr");if(e){const r=e.jumlah_stok!==null&&e.jumlah_stok!==void 0?e.jumlah_stok:"";a.innerHTML=`
            <td>
            ...
            <td><input type="number" class="form-control variant-stock" value="${r}" readonly></td>
            <td>

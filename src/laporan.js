...
// global
let jenisSelect, startDateInput, endDateInput;
let generateBtn, downloadBtn;
let reportContent, generateText, generateSpinner;
...
// input
function setupGroupByDropdown(jenisSelect, groupByContainer) {
    ...
    // Hide groupBy dropdown initially
    ...
    jenisSelect.addEventListener('change', () => {
        if (jenisSelect.value === 'Pembelian' || jenisSelect.value === 'Penjualan') {
            ...
            updateGroupByOptions(jenisSelect.value, groupBySelect);
...
function updateGroupByOptions(reportType, groupBySelect) {
...
function generateByEntity(pdf, data, yPos, margin, options) {
    ...
    pdf.autoTable({
        ...
        alternateRowStyles: {
            fillColor: [255, 255, 255] // no striping table
        },
        columnStyles: {
            2: {halign: 'right'},
            3: {halign: 'right'},
...
"Laba Rugi": {
    fetchData: async (startDate, endDate) => {
        ...
        // operating expense
        const monthly = 135000 * 4; // retribusi/bulan
        const daily = monthly / 30; // per hari
        
        const start = new Date(startDate);
        const end = new Date(endDate);
        
        const diffInDays = Math.floor((end - start) / (1000 * 60 * 60 * 24)) + 1;
        const operatingExpenses = Math.round(daily * diffInDays);
            ...
            items.forEach(item => {
                ...
                if (!productMap.has(productName)) {
                    productMap.set(productName, {
                        ...
                        bakulCount: 0,
                        bakulIds: new Set(),
                        orderIds: new Set()  // Initialize orderIds Set
                    });
                }
                ...
                // Track unique orders
                if (item.pesanan_penjualan?.id) {
                    if (!productData.orderIds.has(item.pesanan_penjualan.id)) {
                ...
                // Track unique bakul
                if (item.pesanan_penjualan?.bakul?.id) {
                    if (!productData.bakulIds.has(item.pesanan_penjualan.bakul.id)) {
                        productData.bakulIds.add(item.pesanan_penjualan.bakul.id);
                        productData.bakulCount++;
                    }
                }
            });
...
async function generateReport() {
    ...
    if (!jenis || !startDate || !endDate) {
        showToast('Harap pilih semua input yang diperlukan (jenis laporan, tanggal mulai, dan tanggal akhir)', 'error');
    ...
    // Additional validation for Pembelian/Penjualan reports
    if ((jenis === 'Pembelian' || jenis === 'Penjualan') && !document.getElementById('groupBy').value) {
        showToast('Harap pilih pengelompokkan untuk laporan ini', 'error');
        disableDownloadButton();
        return;
    }
...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    setupGroupByDropdown(jenisSelect, groupByContainer);

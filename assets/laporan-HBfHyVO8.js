import{s as k}from"./db_conn-CuK1RHLN.js";import{n as rt}from"./navbar-BXvx_Lss.js";import{c as st,d as ot}from"./import-wcpIJIsu.js";fetch("navbar.html").then(y=>y.text()).then(y=>{document.getElementById("navbar").innerHTML=y;const $=document.createElement("script");$.type="module",$.src=rt,document.body.appendChild($)}).catch(y=>console.error("Error loading navbar:",y));const{jsPDF:ut}=window.jspdf;(async()=>await st())();function I(y,$="success"){const P=document.getElementById("toastContainer"),S=document.createElement("div");S.classList.add("toast"),S.setAttribute("role","alert"),S.setAttribute("aria-live","assertive"),S.setAttribute("aria-atomic","true"),S.classList.add($==="success"?"bg-success":"bg-danger"),S.classList.add("text-white");const v=document.createElement("div");v.classList.add("toast-body"),v.textContent=y,S.appendChild(v),P.appendChild(S),new bootstrap.Toast(S,{autohide:!0,delay:3e3}).show(),S.addEventListener("hidden.bs.toast",()=>{S.remove()})}function it(){const y=document.querySelector(".report-container");y&&y.scrollTo({top:0,behavior:"smooth"})}function Y(y){return Array.isArray(y)?y.length===0:typeof y=="object"&&y!==null?y.logs?y.logs.length===0:Object.keys(y).length===0:!1}document.addEventListener("DOMContentLoaded",async()=>{await ot();const y=document.getElementById("jenis"),$=document.getElementById("startDate"),P=document.getElementById("endDate"),S=document.getElementById("generateBtn"),v=document.getElementById("downloadBtn"),A=document.getElementById("report-content"),N=document.getElementById("generateText"),q=document.getElementById("generateSpinner"),T=document.getElementById("groupByContainer"),B=document.createElement("label");B.className="form-label",B.innerHTML="Pengelompokkan",T.appendChild(B);const D=document.createElement("select");D.id="groupBy",D.className="form-select",D.innerHTML="",T.style.display="none",y.addEventListener("change",()=>{y.value==="Pembelian"||y.value==="Penjualan"?(T.style.display="block",Q(y.value),T.appendChild(D)):T.style.display="none"});function Q(e){e==="Pembelian"?D.innerHTML=`
                <option value="" disabled selected>Pilih kelompok</option>
                <option value="order">Pesanan</option>
                <option value="supplier">Supplier</option>
                <option value="product">Produk</option>
            `:e==="Penjualan"&&(D.innerHTML=`
                <option value="" disabled selected>Pilih kelompok</option>
                <option value="order">Pesanan</option>
                <option value="customer">Bakul</option>
                <option value="product">Produk</option>
            `)}const O=new Date,V=new Date(O.getFullYear(),O.getMonth(),1);$.valueAsDate=V,P.valueAsDate=O;const R={"Laba Rugi":{fetchData:async(e,r)=>{const{data:d,error:s}=await k.from("pesanan_penjualan").select("total_dibayarkan").gte("tanggal_pesan",e).lte("tanggal_pesan",r);if(s)throw s;const{data:p,error:t}=await k.from("riwayat_stok").select("hpp, qty").eq("tipe_riwayat","penjualan").gte("tanggal",e).lte("tanggal",r);if(t)throw t;const l=d.reduce((c,u)=>c+(u.total_dibayarkan||0),0);let a;a=p.reduce((c,u)=>c+(u.hpp*u.qty||0),0),a=Math.round(a);const o=l-a,n=135e3*4,i=o-n;return{totalRevenue:l,totalCOGS:a,grossProfit:o,operatingExpenses:n,netProfit:i}},template:(e,r,d)=>{const s=new Date(r).toLocaleDateString("id-ID"),p=new Date(d).toLocaleDateString("id-ID");return`
                    ...
                    <tbody>
                        <tr>
                        <td>Pendapatan</td>
                        <td class="text-end">${b(e.totalRevenue)}</td>
                        </tr>
                        <tr>
                        <td>Harga Pokok Penjualan (HPP)</td>
                        <td class="text-end">${b(e.totalCOGS)}</td>
                        </tr>
                        <tr class="table-active">
                        <td><strong>Laba Kotor</strong></td>
                        <td class="text-end"><strong>${b(e.grossProfit)}</strong></td>
                        </tr>
                        <tr>
                        <td>Biaya Operasional</td>
                        <td class="text-end">${b(e.operatingExpenses)}</td>
                        </tr>
                        <tr class="table-active">
                        <td><strong>Laba Bersih</strong></td>
                        <td class="text-end"><strong>${b(e.netProfit)}</strong></td>
                        </tr>
                    </tbody>
                    </table>
                ...
                `}else if(s==="product"){const l=e.reduce((n,i)=>n+(i.totalSold||0),0),a=e.reduce((n,i)=>n+(i.totalRevenue||0),0),o=e.reduce((n,i)=>n+(i.totalOrders||0),0);return`
                    ...
                    <tbody>
                        ...
                        <td class="text-end"><strong>${l}</strong></td>
                        <td class="text-end"><strong>${b(a)}</strong></td>
                        </tr>
                    </tbody>
                    </table>
                `}else{const l=e.reduce((a,o)=>a+(o.total_dibayarkan||0),0);return`
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    <table class="table table-sm">
                                        ...
                                        <tbody>
                                            ${a.item_pesanan_penjualan.map(n=>{var h;const i=((h=n.varian)==null?void 0:h.harga_standar)||0,c=n.priceDifference||0,u=c===0?"0":c>0?`+${b(c)}`:b(c),g=c>0?"text-success":c<0?"text-danger":"";return`
                                                    <tr>
                ...
                `).gte("tanggal",e).lte("tanggal",r).order("id_varian",{ascending:!0});if(s)throw s;const p=new Map;return d.forEach(t=>{const l=t.varian.id;p.has(l)||p.set(l,{id:l,name:`${t.varian.produk.nama} - ${t.varian.varian}`,history:[],purchaseHistory:[]});const a=p.get(l);a.history.push(t),["penjualan","penyesuaian_keluar"].includes(t.tipe_riwayat)&&(a.hppHistory=a.hppHistory||[],a.hppHistory.push({qty:t.qty,hpp:t.hpp,tanggal:t.tanggal}))}),Array.from(p.values()).map(t=>{const l=t.history.sort((h,m)=>new Date(h.tanggal)-new Date(m.tanggal));let a;if(l.length>0){const h=l[0];a=["purchase","penyesuaian_masuk"].includes(h.tipe_riwayat)?h.saldo-h.qty:h.saldo+h.qty}else a=0;const o=l.filter(h=>h.tipe_riwayat==="pembelian").reduce((h,m)=>h+m.qty,0),n=l.filter(h=>h.tipe_riwayat==="penjualan").reduce((h,m)=>h+m.qty,0),i=l.filter(h=>h.tipe_riwayat==="penyesuaian_masuk").reduce((h,m)=>h+m.qty,0),c=l.filter(h=>h.tipe_riwayat==="penyesuaian_keluar").reduce((h,m)=>h+m.qty,0),u=l.length>0?l[l.length-1].saldo:a;let g=0;return t.hppHistory&&t.hppHistory.length>0&&(t.hppHistory.sort((h,m)=>new Date(m.tanggal)-new Date(h.tanggal)),g=t.hppHistory[0].hpp,console.log(`Latest HPP for ${t.name}`,t.hppHistory[0])),{id:t.id,name:t.name,initialStock:a,purchases:o,sales:n,adjustmentsIn:i,adjustmentsOut:c,finalStock:u,hpp:g}})},template:(e,r,d)=>{const s=new Date(r).toLocaleDateString("id-ID"),p=new Date(d).toLocaleDateString("id-ID");return`    

import"./modulepreload-polyfill-B5Qt9EMX.js";import{n as at}from"./navbar-Cw31nkU0.js";import{s as S}from"./db_conn-C7Nb5uSA.js";fetch("navbar.html").then(f=>f.text()).then(f=>{document.getElementById("navbar").innerHTML=f;const k=document.createElement("script");k.type="module",k.src=at,document.body.appendChild(k)}).catch(f=>console.error("Error loading navbar:",f));const{jsPDF:rt}=window.jspdf;document.addEventListener("DOMContentLoaded",()=>{const f=document.getElementById("jenis"),k=document.getElementById("startDate"),T=document.getElementById("endDate"),P=document.getElementById("generateBtn"),U=document.getElementById("downloadBtn"),j=document.getElementById("report-content"),O=document.getElementById("generateText"),E=document.getElementById("generateSpinner"),w=document.getElementById("groupByContainer"),L=document.createElement("label");L.className="form-label",L.innerHTML="Pengelompokkan",w.appendChild(L);const $=document.createElement("select");$.id="groupBy",$.className="form-select",$.innerHTML="",w.style.display="none",f.addEventListener("change",()=>{f.value==="Pembelian"||f.value==="Penjualan"?(w.style.display="block",G(f.value),w.appendChild($)):w.style.display="none"});function G(a){a==="Pembelian"?$.innerHTML=`
            ...
            `)}const I=new Date,z=new Date(I.getFullYear(),I.getMonth(),1);k.valueAsDate=z,T.valueAsDate=I;const A={"Laba Rugi":{fetchData:async(a,s)=>{const{data:d,error:l}=await S.from("pesanan_penjualan").select("total_dibayarkan").gte("tanggal_pesan",a).lte("tanggal_pesan",s);if(l)throw l;const{data:i,error:e}=await S.from("riwayat_stok").select("hpp, qty").eq("tipe_riwayat","penjualan").gte("tanggal",a).lte("tanggal",s);if(e)throw e;const o=d.reduce((p,u)=>p+(u.total_dibayarkan||0),0);let t;t=i.reduce((p,u)=>p+(u.hpp*u.qty||0),0),t=Math.round(t);const r=o-t,n=r*.3,c=r-n;return{totalRevenue:o,totalCOGS:t,grossProfit:r,operatingExpenses:n,netProfit:c}},template:(a,s,d)=>{const l=new Date(s).toLocaleDateString("id-ID"),i=new Date(d).toLocaleDateString("id-ID");return`
                ...
                `}}},Penjualan:{fetchData:async(a,s)=>{const d=document.getElementById("groupBy").value;if(d==="customer"){const{data:l,error:i}=await S.from("pesanan_penjualan").select(`
                        id,
                        tanggal_pesan,
                        total_dibayarkan,
                        bakul:id_bakul(nama),
                        item_pesanan_penjualan(id_jual, qty_dipesan)
                    `).eq("status_pesanan","selesai").gte("tanggal_pesan",a).lte("tanggal_pesan",s);if(i)throw i;const e=new Map;return l.forEach(o=>{var c,p;const t=((c=o.bakul)==null?void 0:c.nama)||"Walk-in";e.has(t)||e.set(t,{customer:t,totalOrders:0,totalItems:0,totalPaid:0});const r=e.get(t);r.totalOrders++,r.totalPaid+=o.total_dibayarkan||0;const n=((p=o.item_pesanan_penjualan)==null?void 0:p.reduce((u,g)=>u+(g.qty_dipesan||0),0))||0;r.totalItems+=n}),Array.from(e.values())}else if(d==="product"){const{data:l,error:i}=await S.from("item_pesanan_penjualan").select(`
                        qty_dipesan,
                        harga_jual,
                        varian:id_varian(varian, produk:id_produk(nama)),
                ...
                `}}},"Kartu Stok":{fetchData:async(a,s)=>{const{data:d,error:l}=await S.from("riwayat_stok").select(`
                    tanggal,
                    tipe_riwayat,
                    qty,
                    saldo,
            ...
            `}},"Stock Opname":{fetchData:async(a,s)=>{const{data:d,error:l}=await S.from("log_opname").select(`
                    id,
                    tanggal,
                    status_log,
                    item_opname(

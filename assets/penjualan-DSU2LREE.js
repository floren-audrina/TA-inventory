import"./modulepreload-polyfill-B5Qt9EMX.js";import{s as u}from"./db_conn-C7Nb5uSA.js";import{p as Q}from"./import-Cn2uwdsk.js";let L;function D(e){return{dipesan:"secondary",diambil:"success",dibayar:"primary"}[e]||"secondary"}function j(e){return e?new Date(e).toLocaleDateString("id-ID",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"-"}function E(e){return new Intl.NumberFormat("id-ID",{style:"currency",currency:"IDR",minimumFractionDigits:0}).format(e)}function p(e,t="success"){const a=document.getElementById("toastContainer"),n=document.createElement("div");n.className=`toast align-items-center text-white bg-${t==="success"?"success":"danger"} border-0`,n.setAttribute("role","alert"),n.innerHTML=`
        <div class="d-flex">
            <div class="toast-body">${e}</div>
            <button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast"></button>
        </div>
    `,a.appendChild(n),new bootstrap.Toast(n,{autohide:!0,delay:3e3}).show(),n.addEventListener("hidden.bs.toast",()=>n.remove())}async function N(){try{const{data:e,error:t}=await u.from("bakul").select("id, nama").order("nama",{ascending:!0});if(t)throw t;const a=document.getElementById("customer");a.innerHTML='<option value="" disabled selected>Pilih Bakul</option>',e.forEach(n=>{const r=document.createElement("option");r.value=n.id,r.textContent=`${n.nama}`,a.appendChild(r)})}catch(e){console.error("Error loading bakuls:",e),p("Gagal memuat daftar bakul","error")}}async function O(e="add",t=null){L||(L=new bootstrap.Modal(document.getElementById("saleModal")),await N()),document.getElementById("saleForm").reset(),document.querySelector("#productsTable tbody").innerHTML="",document.getElementById("statusFieldsContainer").innerHTML="",J();const a=document.querySelector(".status-toggle-container"),n=document.getElementById("saleStatusBadge"),r=document.getElementById("saleModalLabel"),o=document.getElementById("addProductBtn"),i=document.getElementById("printButtonGroup"),s=document.getElementById("saleModal");if(s.dataset.mode=e,o&&(o.style.display="block"),i&&(i.style.display="none"),e==="edit"&&t){a&&(a.style.display="block"),n&&(n.style.display="none"),r.textContent="Edit Pesanan Penjualan",document.getElementById("saveSaleBtn").textContent="Update",await W(t);const l=s.dataset.currentStatus;o&&(o.style.display=l==="dipesan"?"block":"none")}else{a&&(a.style.display="none"),n&&(n.style.display="inline-block",n.textContent="draft",n.className=`badge bg-${D("dipesan")}`),r.textContent="Tambah Pesanan Penjualan";const l=document.querySelector("#productsTable thead tr");l.innerHTML=`
            <th>Produk</th>
            <th>Varian</th>
            <th>Qty. Dipesan</th>
            <th>Harga</th>
            <th>Subtotal</th>
            <th></th>
        `,document.getElementById("totalAmount").textContent=E(0),s.removeAttribute("data-sale-id"),s.removeAttribute("data-current-status"),document.getElementById("saveSaleBtn").textContent="Simpan"}L.show()}async function W(e){try{const{data:t,error:a}=await u.from("pesanan_penjualan").select(`
                id,
                tanggal_pesan,
                id_bakul,
                status_pesanan,
    ...
    `,t==="dipesan"){const r=a.querySelector(".product-select"),o=a.querySelector(".variant-select");r.addEventListener("change",async function(){await S(r,o)}),setTimeout(async()=>{await S(r,o,a,e.produk_varian.id)},0)}return R(a),a}function R(e){const t=()=>{const n=parseFloat(e.querySelector(".price").value)||0,r=parseInt(e.querySelector(".quantity").value)||0,o=Math.round(n*r);console.log(o);const i=e.querySelector(".subtotal");i.tagName==="TD"?i.textContent=E(o):i.value=o,q()};e.querySelector(".price").addEventListener("input",t),e.querySelector(".quantity").addEventListener("input",t),t();const a=e.querySelector(".remove-product, .remove-item");a&&a.addEventListener("click",()=>{e.remove(),q()})}function q(){let e=0;document.querySelectorAll("#productsTable tbody tr").forEach(t=>{const a=t.querySelector(".subtotal").textContent,n=parseFloat(a.replace(/[^0-9]/g,""))||0;e+=n}),document.getElementById("totalAmount").textContent=E(e)}function C(){const t=document.getElementById("saleModal").dataset.mode==="add";let a;if(t)a="draft";else{const o=document.querySelector("#saleStatusToggle .btn-status.active");a=(o==null?void 0:o.dataset.status)||"dipesan"}const n=o=>o?new Date(o).toISOString():null,r={orderDate:n(document.getElementById("saleDate").value),needDate:n(document.getElementById("needDate").value),bakulID:document.getElementById("customer").value,status:t?"dipesan":a,items:Array.from(document.querySelectorAll("#productsTable tbody tr")).map(o=>({variantId:o.querySelector(".variant-select").value,quantity:parseInt(o.querySelector(".quantity").value)||0,rowId:o.dataset.itemId||null,price:parseFloat(o.querySelector(".price").value)||0}))};return t||(a==="diambil"&&(r.tanggalAmbil=n(document.getElementById("tanggalAmbil").value),r.pihakPengambil=document.getElementById("pihakPengambil").value),a==="dibayar"&&(r.tanggalPembayaran=n(document.getElementById("tanggalPembayaran").value),r.totalDibayarkan=document.getElementById("totalDibayarkan").value,r.alatPembayaran=document.getElementById("alatPembayaran").value)),r}async function V(){const e=document.getElementById("saleModal").dataset.mode;try{e==="edit"?await te():await ee(),L.hide(),await I()}catch(t){console.error("Save failed:",t),p("Gagal menyimpan pesanan","error")}}async function ee(){const e=C();console.log(e);try{for(const r of e.items){const{error:o}=await u.rpc("adjust_reserved_stock",{p_variant_id:r.variantId,p_adjustment:r.quantity});if(o)throw o}const{data:t,error:a}=await u.from("pesanan_penjualan").insert({tanggal_pesan:e.orderDate,tanggal_dibutuhkan:e.needDate,id_bakul:e.bakulID,status_pesanan:"dipesan"}).select().single();if(a)throw a;const{error:n}=await u.from("item_pesanan_penjualan").insert(e.items.map(r=>({id_jual:t.id,id_varian:r.variantId,qty_dipesan:r.quantity,harga_jual:r.price})));if(n)throw n;p("Pesanan baru berhasil dibuat","success"),await I()}catch(t){console.error("Create order error:",t),p("Gagal membuat pesanan: "+t.message,"error")}}async function te(){const e=document.getElementById("saleModal").dataset.saleId,t=C();try{const a={tanggal_pesan:t.orderDate,id_bakul:t.bakulID,status_pesanan:t.status};t.status==="diambil"?(a.tanggal_diambil=t.tanggalAmbil,a.pihak_pengambil=t.pihakPengambil):t.status==="dibayar"&&(a.tanggal_dibayar=t.tanggalPembayaran,a.total_dibayarkan=t.totalDibayarkan,a.alat_pembayaran=t.alatPembayaran);const{error:n}=await u.from("pesanan_penjualan").update(a).eq("id",e);if(n)throw n;const{data:r,error:o}=await u.from("item_pesanan_penjualan").select("id, id_varian, qty_dipesan").eq("id_jual",e);if(o)throw o;const i={};r.forEach(d=>{i[d.id]={id_varian:d.id_varian,qty_dipesan:d.qty_dipesan}});const s=[],l=[],c={};t.items.forEach(d=>{var f;const y={id_jual:e,id_varian:d.variantId,qty_dipesan:d.quantity,harga_jual:d.price};if(d.rowId){s.push({...y,id:d.rowId});const w=((f=i[d.rowId])==null?void 0:f.qty_dipesan)||0,_=d.quantity-w;console.log(_),_!==0&&(c[d.variantId]=(c[d.variantId]||0)+_,console.log(c[d.variantId]))}else l.push(y),c[d.variantId]=(c[d.variantId]||0)+d.quantity});const g=t.items.map(d=>Number(d.rowId)).filter(Boolean),b=r.filter(d=>!g.includes(d.id));console.log(b),console.log("Original items:",r.map(d=>d.id)),console.log("Current items:",t.items.map(d=>d.rowId));for(const d of b)c[d.id_varian]=(c[d.id_varian]||0)-d.qty_dipesan,console.log(c[d.id_varian]);for(const[d,y]of Object.entries(c))if(y!==0){console.log("adj: ",y);const{error:f}=await u.rpc("adjust_reserved_stock",{p_variant_id:d,p_adjustment:y});if(f)throw f}if(b.length>0){const{error:d}=await u.from("item_pesanan_penjualan").delete().in("id",b.map(y=>y.id));if(d)throw d}if(s.length>0){const{error:d}=await u.from("item_pesanan_penjualan").upsert(s);if(d)throw d}if(l.length>0){const{error:d}=await u.from("item_pesanan_penjualan").insert(l);if(d)throw d}p("Pesanan berhasil diperbarui","success"),await I()}catch(a){console.error("Update order error:",a),p("Gagal memperbarui pesanan: "+a.message,"error")}}async function ae(e){if(confirm("Apakah Anda yakin ingin menghapus penjualan ini?"))try{await u.from("item_pesanan_penjualan").delete().eq("id_jual",e),await u.from("pesanan_penjualan").delete().eq("id",e),p("Penjualan berhasil dihapus","success")}catch(t){console.error("Error deleting sale:",t),p("Gagal menghapus penjualan","error")}}async function I(){try{const e=document.getElementById("ongoingOrdersContainer");e.innerHTML=`
        ...
        `,w.appendChild(h)}),l.appendChild(c),l.appendChild(g),l.appendChild(b),l.appendChild(d),l.appendChild(f),l.appendChild(w);const _=document.createElement("div");if(_.className="card-footer bg-transparent border-top-0",e.status_pesanan==="dipesan"){const m=document.createElement("div");m.className="form-check mb-2";const h=document.createElement("input");h.type="checkbox",h.className="form-check-input preparation-checkbox",h.id=`prep-${e.id}`,h.checked=e.status_persiapan||!1;const v=document.createElement("label");v.className="form-check-label",v.htmlFor=`prep-${e.id}`,v.textContent="Persiapan Selesai",re(h,e),m.appendChild(h),m.appendChild(v),_.appendChild(m)}const B=document.createElement("div");B.className="d-flex justify-content-between";const P=document.createElement("button");if(P.className="btn btn-sm",e.status_pesanan==="dipesan"){const m=document.createElement("button");m.className="btn btn-sm btn-outline-danger ms-2 cancel-order",m.innerHTML='<i class="bi bi-x-circle"></i> Batalkan',m.dataset.orderId=e.id;const h=e.items.map(v=>{var k;return{variantId:((k=v.produk_varian)==null?void 0:k.id)||"",qty:v.qty_dipesan||0,price:v.harga_jual||0}});m.dataset.items=JSON.stringify(h),_.appendChild(m)}else if(e.status_pesanan==="dibayar"){const m=document.createElement("button");m.className="btn btn-sm btn-dark ms-2 complete-order",m.innerHTML='<i class="bi bi-check-circle"></i> Selesaikan',m.dataset.orderId=e.id;const h=e.items.map(k=>{var T;return{variantId:((T=k.produk_varian)==null?void 0:T.id)||"",qty:k.qty_dipesan||0,price:k.harga_jual||0}});m.dataset.items=JSON.stringify(h),_.appendChild(m);const v=document.createElement("button");v.className="btn btn-sm btn-outline-primary ms-2 reprint-order",v.innerHTML='<i class="bi bi-printer"></i> Cetak',v.dataset.orderId=e.id,v.addEventListener("click",k=>{k.stopPropagation(),M(e.id)}),_.appendChild(v)}return B.appendChild(P),_.appendChild(B),a.appendChild(n),a.appendChild(l),a.appendChild(_),t.appendChild(a),t}function re(e,t){e.addEventListener("change",async function(){const a=this.checked;if(!confirm(`Yakin ingin ${a?"menandai":"membatalkan tanda"} persiapan selesai untuk pesanan #${t.id}?`)){this.checked=!a;return}this.disabled=!0;try{const{error:o}=await u.from("pesanan_penjualan").update({status_persiapan:a}).eq("id",t.id);if(o)throw o;p(`Status persiapan ${a?"diselesaikan":"dibatalkan"}`,"success");const i=this.closest(".card").querySelector(".prep-status");i&&(i.textContent=`Persiapan: ${a?"Siap":"Belum siap"}`)}catch(o){console.error("Update error:",o),this.checked=!a,p("Gagal memperbarui status","error")}finally{this.disabled=!1}})}async function oe(){const e=document.querySelector("#history tbody");if(e)try{const{data:t,error:a}=await u.from("pesanan_penjualan").select(`
            ...
            `).eq("status_pesanan","selesai").order("tanggal_pesan",{ascending:!1});if(a)throw a;e.innerHTML="",t&&t.length>0?(t.forEach(n=>{const r=new Date(n.tanggal_pesan).toLocaleDateString("id-ID"),o=n.tanggal_diambil?new Date(n.tanggal_diambil).toLocaleDateString("id-ID"):"-",i=n.tanggal_dibayar?new Date(n.tanggal_dibayar).toLocaleDateString("id-ID"):"-",s=n.items.reduce((c,g)=>c+g.qty_dipesan,0),l=document.createElement("tr");l.innerHTML=`
                ...
                `,e.appendChild(l)}),document.querySelectorAll(".reprint-btn").forEach(n=>{n.addEventListener("click",async r=>{const o=r.target.dataset.orderId;try{await M(o)}catch(i){console.error("Error printing receipt:",i),p("Gagal mencetak struk","error")}})}),document.querySelectorAll(".detail-btn").forEach(n=>{n.addEventListener("click",async r=>{const o=r.target.dataset.orderId;await ie(o)})})):e.innerHTML=`
                <tr>
                    <td colspan="11" class="text-center text-muted">Tidak ada riwayat pesanan</td>
                </tr>
            `}catch(t){console.error("Error fetching history:",t),p("Gagal memuat riwayat pesanan","error")}}async function ie(e){try{const{data:t,error:a}=await u.from("pesanan_penjualan").select(`
                id,
                tanggal_pesan,
                bakul: id_bakul(nama),
                status_pesanan,
        ...
        `,o=document.createElement("div");o.innerHTML=r,document.body.appendChild(o),new bootstrap.Modal(document.getElementById("orderDetailModal")).show(),document.getElementById("orderDetailModal").addEventListener("hidden.bs.modal",()=>{o.remove()})}catch(t){console.error("Error showing order details:",t),p("Gagal memuat detail pesanan","error")}}function de(){var n;const e=document.getElementById("printReceiptBtn"),t=document.getElementById("exportPdfBtn"),a=document.getElementById("printButtonGroup");e==null||e.addEventListener("click",()=>M()),t==null||t.addEventListener("click",se),(n=document.getElementById("saleStatusToggle"))==null||n.addEventListener("click",r=>{if(r.target.closest(".btn-status")){const o=r.target.closest(".btn-status").dataset.status;a.style.display=o==="dibayar"?"flex":"none"}})}async function M(e=null){let t;if(e?t=e:t=document.getElementById("saleModal").dataset.saleId,!t){p("Tidak ada pesanan yang dipilih","error");return}try{let a=await z(t);if(!a.tanggal_dibayar||!a.total_dibayarkan||!a.alat_pembayaran){const r=C();if(r)a={...a,tanggal_dibayar:r.tanggalPembayaran,total_dibayarkan:parseFloat(r.totalDibayarkan),alat_pembayaran:r.alatPembayaran};else throw new Error("Data pembayaran belum lengkap")}const n=U(a);le(n)}catch(a){console.error("Error printing receipt:",a),p(a.message||"Gagal mencetak struk","error")}}async function se(){const e=document.getElementById("saleModal").dataset.saleId;if(!e){p("Tidak ada pesanan yang dipilih","error");return}const t=document.getElementById("exportPdfBtn"),a=t.innerHTML;try{t.innerHTML='<span class="spinner-border spinner-border-sm" role="status"></span> Memproses...',t.disabled=!0;const n=await z(e),r=U(n),{jsPDF:o}=window.jspdf;await new o({orientation:"portrait",unit:"mm",format:[80,297]}).html(r,{html2canvas:{scale:.3,logging:!1,useCORS:!0,letterRendering:!0,width:80},callback:function(s){s.save(`Struk_${e}.pdf`),t.innerHTML=a,t.disabled=!1},x:0,y:0,width:80,windowWidth:800})}catch(n){console.error("PDF export error:",n),p("Gagal membuat PDF: "+n.message,"error"),t.innerHTML=a,t.disabled=!1}}function le(e){const t=window.open("","_blank","width=800,height=600"),a=(e.match(/<tr>/g)||[]).length,n=50,r=a*3,o=Math.max(n+r,150),i="5mm 3mm";t.document.open(),t.document.write(`
        <!DOCTYPE html>
        <html>
        <head>

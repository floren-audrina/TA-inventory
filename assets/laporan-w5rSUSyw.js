import{s as k}from"./db_conn-Cc8gtU8M.js";import{n as rt,d as st}from"./import-BouQIIWv.js";import{c as ot}from"./auth-B14p8eo6.js";fetch("navbar.html").then(y=>y.text()).then(y=>{document.getElementById("navbar").innerHTML=y;const $=document.createElement("script");$.type="module",$.src=rt,document.body.appendChild($)}).catch(y=>console.error("Error loading navbar:",y));const{jsPDF:ut}=window.jspdf;(async()=>await ot())();function I(y,$="success"){const P=document.getElementById("toastContainer"),S=document.createElement("div");S.classList.add("toast"),S.setAttribute("role","alert"),S.setAttribute("aria-live","assertive"),S.setAttribute("aria-atomic","true"),S.classList.add($==="success"?"bg-success":"bg-danger"),S.classList.add("text-white");const v=document.createElement("div");v.classList.add("toast-body"),v.textContent=y,S.appendChild(v),P.appendChild(S),new bootstrap.Toast(S,{autohide:!0,delay:3e3}).show(),S.addEventListener("hidden.bs.toast",()=>{S.remove()})}function it(){const y=document.querySelector(".report-container");y&&y.scrollTo({top:0,behavior:"smooth"})}function Y(y){return Array.isArray(y)?y.length===0:typeof y=="object"&&y!==null?y.logs?y.logs.length===0:Object.keys(y).length===0:!1}document.addEventListener("DOMContentLoaded",async()=>{await st();const y=document.getElementById("jenis"),$=document.getElementById("startDate"),P=document.getElementById("endDate"),S=document.getElementById("generateBtn"),v=document.getElementById("downloadBtn"),A=document.getElementById("report-content"),N=document.getElementById("generateText"),q=document.getElementById("generateSpinner"),T=document.getElementById("groupByContainer"),B=document.createElement("label");B.className="form-label",B.innerHTML="Pengelompokkan",T.appendChild(B);const D=document.createElement("select");D.id="groupBy",D.className="form-select",D.innerHTML="",T.style.display="none",y.addEventListener("change",()=>{y.value==="Pembelian"||y.value==="Penjualan"?(T.style.display="block",Q(y.value),T.appendChild(D)):T.style.display="none"});function Q(e){e==="Pembelian"?D.innerHTML=`

import"./modulepreload-polyfill-B5Qt9EMX.js";import{n as f}from"./navbar-Cw31nkU0.js";import{s as l}from"./db_conn-C7Nb5uSA.js";import{p as v}from"./import-Cn2uwdsk.js";fetch("navbar.html").then(e=>e.text()).then(e=>{document.getElementById("navbar").innerHTML=e;const a=document.createElement("script");a.type="module",a.src=f,document.body.appendChild(a)}).catch(e=>console.error("Error loading navbar:",e));function s(e,a="success"){const t=document.getElementById("toastContainer"),n=document.createElement("div");n.classList.add("toast"),n.setAttribute("role","alert"),n.setAttribute("aria-live","assertive"),n.setAttribute("aria-atomic","true"),n.classList.add(a==="success"?"bg-success":"bg-danger"),n.classList.add("text-white");const r=document.createElement("div");r.classList.add("toast-body"),r.textContent=e,n.appendChild(r),t.appendChild(n),new bootstrap.Toast(n,{autohide:!0,delay:3e3}).show(),n.addEventListener("hidden.bs.toast",()=>{n.remove()})}function c(){return new URLSearchParams(window.location.search).get("variantId")}async function h(e){const{data:a,error:t}=await l.from("produk_varian").select(`
            *,
            produk:id_produk(*)
        `).eq("id",e).single();return t?(console.error("Error fetching variant details:",t),null):a}async function w(){const e=document.getElementById("tipe_riwayat");e.innerHTML='<option value="">Memuat tipe riwayat...</option>';try{const{data:a,error:t}=await l.rpc("get_enum_values",{enum_name:"tipe_riwayat"});if(t)throw t;e.innerHTML='<option value="" disabled selected>Pilih tipe riwayat</option>',a.filter(r=>r.value==="penyesuaian_masuk"||r.value==="penyesuaian_keluar").forEach(r=>{const d=document.createElement("option");d.value=r.value,d.text=r.value.charAt(0).toUpperCase()+r.value.slice(1).replace("_"," "),e.appendChild(d)})}catch(a){console.error("Error fetching tipe_riwayat enum:",a),e.innerHTML='<option value="">Error loading tipe riwayat</option>',s("Gagal memuat tipe riwayat","error")}}async function E(e){e.preventDefault();const a=c();document.getElementById("tanggal").value;const t=document.getElementById("tipe_riwayat").value,n=parseInt(document.getElementById("qty").value,10);if(document.getElementById("harga")&&parseFloat(document.getElementById("harga").value),!a||!t||!n){alert("Harap isi semua field yang diperlukan.");return}await v({variantId:a,quantity:n})}async function y(e){const a=document.getElementById("historyTableBody");try{if(a.innerHTML=`
            <tr>
            ...
            </tr>`,!e){a.innerHTML='<tr><td colspan="8" class="text-center text-danger">Tidak ditemukan ID varian dalam URL.</td></tr>';return}const t=await h(e);if(!t){a.innerHTML='<tr><td colspan="8" class="text-center text-danger">Varian tidak ditemukan.</td></tr>';return}const n=document.querySelector("h3");n&&(n.textContent=`Riwayat stok untuk "${t.produk.nama}" - ${t.varian}`);const r=document.getElementById("currentStock");r&&(r.textContent=`Stok saat ini: ${t.jumlah_stok}`);const{data:d,error:u}=await l.from("riwayat_stok").select("*").eq("id_varian",e).order("tanggal",{ascending:!1});if(u)throw u;if(console.log("History data:",d),!d||d.length===0){a.innerHTML='<tr><td colspan="8" class="text-center">Tidak ada riwayat stok yang ditemukan untuk varian ini.</td></tr>';return}a.innerHTML="",d.forEach(o=>{const m=document.createElement("tr");m.innerHTML=`
                <td>${o.id}</td>
        ...
        </tr>`;try{let t=l.from("riwayat_stok").select("*").eq("id_varian",a).order("tanggal",{ascending:!1});i.type==="tipe"&&i.tipe&&(t=t.eq("tipe_riwayat",i.tipe));const{data:n,error:r}=await t;if(r)throw r;I(n)}catch(t){console.error("Error filtering entries:",t),s("Gagal memfilter entri","error"),e.innerHTML=`
            <tr>
                <td colspan="8" class="text-center text-danger">
                    Gagal memuat entri riwayat
                </td>
            </tr>`}}function I(e){const a=document.getElementById("historyTableBody");if(a.innerHTML="",!e||e.length===0){a.innerHTML=`
            <tr>

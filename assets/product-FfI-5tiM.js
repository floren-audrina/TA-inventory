import"./modulepreload-polyfill-B5Qt9EMX.js";import{n as F}from"./navbar-Cw31nkU0.js";import{s as d}from"./db_conn-C7Nb5uSA.js";fetch("navbar.html").then(e=>e.text()).then(e=>{document.getElementById("navbar").innerHTML=e;const t=document.createElement("script");t.type="module",t.src=F,document.body.appendChild(t)}).catch(e=>console.error("Error loading navbar:",e));let T,B=null,o={type:null,category:null,subcategory:null,supplier:null,variant:null};const V=new Map([[.25,"1/4 lusin"],[.5,"1/2 lusin"],[1,"1 lusin"]]);function G(){var e,t;document.querySelectorAll('input[name="filterGroup"]').forEach(a=>{a.addEventListener("click",function(r){if(this.checked&&o.type===this.value){this.checked=!1,I();return}o.type=this.value,j()})}),(e=document.getElementById("applyFilter"))==null||e.addEventListener("click",P),(t=document.getElementById("resetFilter"))==null||t.addEventListener("click",I)}async function j(){var t,a;const e=document.getElementById("filterOptionsContainer");switch(e.innerHTML="",document.getElementById("filterCard").classList.remove("d-none"),o.type){case"kategori":e.innerHTML=`
                <div class="mb-3">
                ...
                <div class="card-footer bg-light">
                    <button class="btn btn-sm btn-primary me-2" id="applyFilter">Terapkan</button>
                    <button class="btn btn-sm btn-outline-secondary" id="resetFilter">Reset</button>
                </div>`;break}(t=document.getElementById("applyFilter"))==null||t.addEventListener("click",P),(a=document.getElementById("resetFilter"))==null||a.addEventListener("click",I)}async function A(){try{const{data:e,error:t}=await d.from("kategori").select("id, kategori").order("kategori",{ascending:!0});if(t)throw t;const a=document.getElementById("filterCategory");a.innerHTML='<option value="">Pilih Kategori</option>',e.forEach(r=>{const n=document.createElement("option");n.value=r.id,n.textContent=r.kategori,a.appendChild(n)}),a.addEventListener("change",function(){const r=document.getElementById("filterSubcategory");r.disabled=!this.value,this.value?R(this.value):r.innerHTML='<option value="">Pilih Subkategori</option>'})}catch(e){console.error("Error loading categories:",e),g("Gagal memuat daftar kategori","error")}}async function q(){const e=document.getElementById("productTableBody");try{e.innerHTML=`
            <tr>
            ...
            `).not("id_supplier","is",null).order("id",{ascending:!0}),u=(n==null?void 0:n.filter(l=>{var c,m,k,f;return((m=(c=l.supplier)==null?void 0:c.perusahaan)==null?void 0:m.toLowerCase().includes(e))||((f=(k=l.supplier)==null?void 0:k.cp)==null?void 0:f.toLowerCase().includes(e))}))||[],{data:h,error:p}=await d.from("produk").select(`
                ...
                subkategori!inner(id, subkategori, kategori!inner(id, kategori)),
                supplier:supplier (perusahaan, cp)
            `).not("id_subkategori","is",null).order("id",{ascending:!0}),i=(h==null?void 0:h.filter(l=>{var c,m,k,f,C;return((m=(c=l.subkategori)==null?void 0:c.subkategori)==null?void 0:m.toLowerCase().includes(e))||((C=(f=(k=l.subkategori)==null?void 0:k.kategori)==null?void 0:f.kategori)==null?void 0:C.toLowerCase().includes(e))}))||[];let v=[...a||[],...u,...i].reduce((l,c)=>(l.some(m=>m.id===c.id)||l.push(c),l),[]);if(o.type)switch(o.type){case"kategori":o.category&&(v=v.filter(l=>{var c,m;return o.subcategory?l.id_subkategori===o.subcategory:((m=(c=l.subkategori)==null?void 0:c.kategori)==null?void 0:m.id)===o.category}));break;case"supplier":o.supplier&&(v=v.filter(l=>l.id_supplier===o.supplier));break;case"varian":break}const{data:$,error:M}=await d.from("produk_varian").select("*");if(M)throw M;const S=$.reduce((l,c)=>(l[c.id_produk]||(l[c.id_produk]=[]),l[c.id_produk].push(c),l),{});o.type==="varian"&&o.variant&&(v=v.filter(l=>{const c=S[l.id]||[],m=c.length>1||c.length===1&&c[0].varian.toLowerCase()!=="standar";return o.variant==="has"?m:!m})),H(v,S)}catch(t){console.error("Error searching products:",t),g("Gagal melakukan pencarian","error"),document.getElementById("productTableBody").innerHTML=`
            <tr>
                <td colspan="7" class="text-center text-danger">
                    Gagal memuat data produk
                </td>
            ...
            <td>${V.get(r.qty_minimum)||r.qty_minimum}</td>
            <td>
        ...
        `}function K(){document.getElementById("searchInput").value="",L(),q()}async function x(e="add",t=null){B=t,document.getElementById("productModal"),document.getElementById("productForm").reset(),document.getElementById("variantsTableBody").innerHTML="",document.getElementById("productModalLabel").textContent=e==="add"?"Tambah Produk":"Edit Produk",document.getElementById("saveProductBtn").textContent=e==="add"?"Simpan":"Update",e==="edit"&&t?await O(t):E(),T.show()}async function O(e){try{const{data:t,error:a}=await d.from("produk").select(`
                id, 
                nama, 
                id_subkategori, 
                id_supplier,

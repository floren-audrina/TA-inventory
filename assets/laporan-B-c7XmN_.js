import"./modulepreload-polyfill-B5Qt9EMX.js";import{n as rt,c as st}from"./auth-BddM_kwX.js";import{s as k}from"./db_conn-C7Nb5uSA.js";fetch("navbar.html").then(y=>y.text()).then(y=>{document.getElementById("navbar").innerHTML=y;const $=document.createElement("script");$.type="module",$.src=rt,document.body.appendChild($)}).catch(y=>console.error("Error loading navbar:",y));const{jsPDF:pt}=window.jspdf;(async()=>await st())();function I(y,$="success"){const P=document.getElementById("toastContainer"),S=document.createElement("div");S.classList.add("toast"),S.setAttribute("role","alert"),S.setAttribute("aria-live","assertive"),S.setAttribute("aria-atomic","true"),S.classList.add($==="success"?"bg-success":"bg-danger"),S.classList.add("text-white");const v=document.createElement("div");v.classList.add("toast-body"),v.textContent=y,S.appendChild(v),P.appendChild(S),new bootstrap.Toast(S,{autohide:!0,delay:3e3}).show(),S.addEventListener("hidden.bs.toast",()=>{S.remove()})}function ot(){const y=document.querySelector(".report-container");y&&y.scrollTo({top:0,behavior:"smooth"})}function Y(y){return Array.isArray(y)?y.length===0:typeof y=="object"&&y!==null?y.logs?y.logs.length===0:Object.keys(y).length===0:!1}document.addEventListener("DOMContentLoaded",()=>{const y=document.getElementById("jenis"),$=document.getElementById("startDate"),P=document.getElementById("endDate"),S=document.getElementById("generateBtn"),v=document.getElementById("downloadBtn"),A=document.getElementById("report-content"),N=document.getElementById("generateText"),q=document.getElementById("generateSpinner"),T=document.getElementById("groupByContainer"),B=document.createElement("label");B.className="form-label",B.innerHTML="Pengelompokkan",T.appendChild(B);const D=document.createElement("select");D.id="groupBy",D.className="form-select",D.innerHTML="",T.style.display="none",y.addEventListener("change",()=>{y.value==="Pembelian"||y.value==="Penjualan"?(T.style.display="block",Q(y.value),T.appendChild(D)):T.style.display="none"});function Q(e){e==="Pembelian"?D.innerHTML=`
            ...
            `}}};function b(e){return new Intl.NumberFormat("id-ID",{style:"currency",currency:"IDR",minimumFractionDigits:0}).format(e).replace("Rp","Rp ")}async function J(){const e=y.value,r=$.value,d=P.value;if(!r||!d){I("Harap pilih rentang tanggal","error"),w();return}if(new Date(r)>new Date(d)){I("Tanggal mulai tidak boleh lebih besar dari tanggal akhir","error"),w();return}try{N.textContent="Memuat...",q.style.display="inline-block",S.disabled=!0,w();const s=await R[e].fetchData(r,d);if(Y(s)){A.innerHTML=`
                ...
                `,w();return}const p=R[e].template(s,r,d);A.innerHTML=p,X(),setTimeout(ot,100)}catch(s){console.error("Error generating report:",s),A.innerHTML=`

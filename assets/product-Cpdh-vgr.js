import{s as d}from"./db_conn-Cc8gtU8M.js";import{n as $,d as V}from"./import-BouQIIWv.js";import{c as G}from"./auth-CnZ2l4Iw.js";fetch("navbar.html").then(e=>e.text()).then(async e=>{document.getElementById("navbar").innerHTML=e;const t=await import($);t.setupLogout?t.setupLogout():console.error("setupLogout not found in navbar.js")}).catch(e=>console.error("Error loading navbar:",e));(async()=>await G())();let T,B=null,o={type:null,category:null,subcategory:null,supplier:null,variant:null};const A=new Map([[.25,"1/4 lusin"],[.5,"1/2 lusin"],[1,"1 lusin"]]);function j(){var e,t;document.querySelectorAll('input[name="filterGroup"]').forEach(a=>{a.addEventListener("click",function(r){if(this.checked&&o.type===this.value){this.checked=!1,I();return}o.type=this.value,R()})}),(e=document.getElementById("applyFilter"))==null||e.addEventListener("click",H),(t=document.getElementById("resetFilter"))==null||t.addEventListener("click",I)}async function R(){var t,a;const e=document.getElementById("filterOptionsContainer");switch(e.innerHTML="",document.getElementById("filterCard").classList.remove("d-none"),o.type){case"kategori":e.innerHTML=`
                <div class="mb-3">
        ...
        `}function N(){document.getElementById("searchInput").value="",L(),q()}async function x(e="add",t=null){B=t,document.getElementById("productModal"),document.getElementById("productForm").reset(),document.getElementById("variantsTableBody").innerHTML="",document.getElementById("productModalLabel").textContent=e==="add"?"Tambah Produk":"Edit Produk",document.getElementById("saveProductBtn").textContent=e==="add"?"Simpan":"Update",e==="edit"&&t?await U(t):w(),T.show()}async function U(e){try{const{data:t,error:a}=await d.from("produk").select(`
                id, 
                nama, 
                id_subkategori, 
                id_supplier,
    ...
    `,a.appendChild(r),new bootstrap.Toast(r,{autohide:!0,delay:3e3}).show(),r.addEventListener("hidden.bs.toast",()=>{r.remove()})}function ne(e){window.location.href=`history.html?variantId=${e}`}document.addEventListener("DOMContentLoaded",async()=>{T=new bootstrap.Modal(document.getElementById("productModal")),j(),await V(),document.getElementById("addProductBtn").addEventListener("click",()=>x("add")),document.getElementById("addVariantBtn").addEventListener("click",()=>w()),document.getElementById("saveProductBtn").addEventListener("click",z);const e=document.getElementById("searchInput");document.getElementById("searchButton").addEventListener("click",()=>{e.value.trim()&&document.getElementById("searchIcon").innerHTML.includes("bi-x")?N():_()}),e.addEventListener("keyup",a=>{L(),a.key==="Enter"&&_()}),e.addEventListener("input",L),await Promise.all([X(),D(),te(),E()]),document.getElementById("variantsTableBody").addEventListener("click",a=>{a.target.classList.contains("remove-variant")&&a.target.closest("tr").remove()})});window.editProduct=async function(e){await x("edit",e)};window.deleteProduct=ae;window.viewVariantHistory=ne;window.deleteVariant=re;

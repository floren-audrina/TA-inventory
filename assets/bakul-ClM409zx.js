import{s as l}from"./db_conn-CuK1RHLN.js";import{n as I}from"./navbar-BXvx_Lss.js";import{c as L,d as T}from"./import-wcpIJIsu.js";fetch("navbar.html").then(t=>t.text()).then(t=>{document.getElementById("navbar").innerHTML=t;const e=document.createElement("script");e.type="module",e.src=I,document.body.appendChild(e)}).catch(t=>console.error("Error loading navbar:",t));(async()=>await L())();function M(){const t=document.getElementById("bakulForm");t&&(t.reset(),t.dataset.bakulId="",document.getElementById("bakulModalLabel").textContent="Tambah Bakul",document.getElementById("submitButton").textContent="Tambah")}function d(t,e="success"){const a=document.getElementById("toastContainer"),n=document.createElement("div");n.classList.add("toast"),n.setAttribute("role","alert"),n.setAttribute("aria-live","assertive"),n.setAttribute("aria-atomic","true"),n.classList.add(e==="success"?"bg-success":"bg-danger"),n.classList.add("text-white");const o=document.createElement("div");o.classList.add("toast-body"),o.textContent=t,n.appendChild(o),a.appendChild(n),new bootstrap.Toast(n,{autohide:!0,delay:3e3}).show(),n.addEventListener("hidden.bs.toast",()=>{n.remove()})}async function x(){const t=document.getElementById("kota");if(!t){console.warn("Kota dropdown not found in the DOM.");return}try{const{data:e,error:a}=await l.from("kota").select("id, kota");if(a)throw a;t.innerHTML='<option value="" disabled selected>Pilih Kota</option>',e.forEach(n=>{const o=document.createElement("option");o.value=n.id,o.text=n.kota,t.appendChild(o)})}catch(e){console.error("Error fetching cities:",e.message),d("Failed to load cities. Please try again.","error")}}async function _(){const t=document.getElementById("nama").value,e=document.getElementById("no_hp").value,a=document.getElementById("kota").value,n=document.getElementById("bakulForm").dataset.bakulId;try{if(n){const{data:s,error:i}=await l.from("bakul").update({nama:t,no_hp:e,id_kota:a}).eq("id",n);if(i)throw i;d("Data bakul berhasil diperbarui!","success")}else{const{data:s,error:i}=await l.from("bakul").insert([{nama:t,no_hp:e,id_kota:a}]);if(i)throw i;d("Data bakul berhasil ditambahkan!","success")}bootstrap.Modal.getInstance(document.getElementById("bakulModal")).hide(),await y()}catch(o){console.error("Error saving supplier:",o.message),d("Gagal menyimpan data bakul, silahkan coba lagi.","error")}}async function C(t){try{const{data:e,error:a}=await l.from("bakul").select("*").eq("id",t).single();if(a)throw a;document.getElementById("nama").value=e.nama,document.getElementById("no_hp").value=e.no_hp,document.getElementById("kota").value=e.id_kota,document.getElementById("bakulModalLabel").textContent="Edit Bakul",document.getElementById("submitButton").textContent="Edit",document.getElementById("bakulForm").dataset.bakulId=t,new bootstrap.Modal(document.getElementById("bakulModal")).show()}catch(e){console.error("Error fetching supplier:",e.message),d("Gagal mengambil data bakul, silahkan coba lagi.","error")}}async function F(t){if(confirm("Apakah Anda yakin ingin menghapus data bakul ini?"))try{const{data:e,error:a}=await l.from("bakul").delete().eq("id",t);if(a)throw a;await y(),d("Data bakul berhasil dihapus!","success")}catch(e){console.error("Error deleting supplier:",e.message),d("Gagal menghapus data bakul, silahkan coba lagi.","error")}}async function y(){const t=document.getElementById("bakulTableBody");if(t)try{t.innerHTML=`
            ...
            `,t.appendChild(o)})}catch(e){console.error("Error fetching bakul:",e.message),d("Gagal menampilkan data bakul, silahkan coba lagi.","error")}}const r={type:null,kota:null};function H(){var e,a;const t=document.getElementById("filterKota");t&&t.addEventListener("click",function(n){if(this.checked&&r.type==="kota"){this.checked=!1,h();return}r.type="kota",$()}),(e=document.getElementById("applyFilter"))==null||e.addEventListener("click",f),(a=document.getElementById("resetFilter"))==null||a.addEventListener("click",h)}async function $(){var e,a;const t=document.getElementById("filterOptionsContainer");t.innerHTML="",document.getElementById("filterCard").classList.remove("d-none"),t.innerHTML=`
        <div class="mb-3">
        ...
        <div class="card-footer bg-light">
            ...
            <tr>
                ...
                no_hp,
                kota:id_kota(kota)
            `).order("id",{ascending:!0});r.type==="kota"&&r.kota&&(a=a.eq("id_kota",r.kota));const n=a;t&&n.or(`nama.ilike.%${t}%,no_hp.ilike.%${t}%`);const{data:o,error:s}=await n;let i=[];if(t&&!(r.type==="kota"&&r.kota)){const{data:c,error:u}=await l.from("kota").select("id, kota").ilike("kota",`%${t}%`);if(!u&&(c==null?void 0:c.length)>0){const m=c.map(w=>w.id),{data:k,error:v}=await a.in("id_kota",m);v||(i=k||[])}}let p;t?p=[...o||[],...i].reduce((u,m)=>(u.some(k=>k.id===m.id)||u.push(m),u),[]):p=o||[],B(p)}catch(e){console.error("Error searching bakuls:",e),d("Gagal melakukan pencarian bakul","error"),document.getElementById("bakulTableBody").innerHTML=`
            <tr>
                <td colspan="5" class="text-center text-danger">
                    Gagal memuat data bakul
                </td>
        ...
        `,e.appendChild(n)})}function g(){const t=document.getElementById("searchInput"),e=document.getElementById("searchIcon");t.value.trim()?e.innerHTML=`
            ...
            </svg>`}function K(){document.getElementById("searchInput").value="",g(),E()}document.addEventListener("DOMContentLoaded",async()=>{await T(),await x(),await y(),H();const t=document.getElementById("bakulForm");t&&t.addEventListener("submit",async o=>{o.preventDefault(),await _()});const e=document.getElementById("bakulModal");e&&e.addEventListener("hidden.bs.modal",()=>{M()});const a=document.getElementById("searchInput"),n=document.getElementById("searchButton");n&&a&&(n.addEventListener("click",()=>{a.value.trim()&&document.getElementById("searchIcon").innerHTML.includes("bi-x")?K():b()}),a.addEventListener("keyup",o=>{g(),o.key==="Enter"&&b()}),a.addEventListener("input",g))});window.editBakul=C;window.deleteBakul=F;

import{s as y}from"./db_conn-Cc8gtU8M.js";import{n as K,p as X,d as Z}from"./import-BouQIIWv.js";import{c as V}from"./auth-B14p8eo6.js";fetch("navbar.html").then(e=>e.text()).then(e=>{document.getElementById("navbar").innerHTML=e;const t=document.createElement("script");t.type="module",t.src=K,document.body.appendChild(t)}).catch(e=>console.error("Error loading navbar:",e));(async()=>await V())();let x;function F(e){return{dipesan:"secondary",diambil:"success",dibayar:"primary"}[e]||"secondary"}function q(e){return e?new Date(e).toLocaleDateString("id-ID",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"-"}function C(e){if(!e)return"";const t=new Date(e);return new Date(t.getTime()-t.getTimezoneOffset()*6e4).toISOString().slice(0,16)}function I(e){return new Intl.NumberFormat("id-ID",{style:"currency",currency:"IDR",minimumFractionDigits:0}).format(e)}function m(e,t="success"){const a=document.getElementById("toastContainer"),n=document.createElement("div");n.className=`toast align-items-center text-white bg-${t==="success"?"success":"danger"} border-0`,n.setAttribute("role","alert"),n.innerHTML=`
        <div class="d-flex">
    ...
    `}async function Y(e){const{data:t,error:a}=await y.from("pesanan_penjualan").select(`
        ...
        `).eq("id",e).single();if(a)throw a;return t}document.addEventListener("DOMContentLoaded",async()=>{x=new bootstrap.Modal(document.getElementById("saleModal"));const e=document.getElementById("penjualanTabs"),t=document.getElementById("addSaleBtn"),a=document.getElementById("ongoing-tab"),n=document.getElementById("history-tab");e.addEventListener("click",function(i){i.target&&i.target.matches("#history-tab")?t.style.display="none":i.target&&i.target.matches("#ongoing-tab")&&(t.style.display="inline-block")}),await U(),await l(),document.getElementById("addProductBtn").addEventListener("click",W),document.addEventListener("click",async i=>{if(i.target.closest(".cancel-order")){const c=i.target.closest(".cancel-order"),s=c.dataset.orderId,h=JSON.parse(c.dataset.items||"[]");console.log("data: ",h);try{if(!confirm("Yakin ingin membatalkan pesanan ini?"))return;for(const d of h){console.log("item: ",d);const{error:u}=await y.rpc("adjust_reserved_stock",{p_variant_id:d.variantId,p_adjustment:-d.qty});if(u)throw u;console.log("stok_reservasi updated")}const{error:b}=await y.from("pesanan_penjualan").delete().eq("id",s);if(b)throw b;m("Pesanan dibatalkan","success"),B()}catch(b){console.error("Order processing failed:",b),m("Gagal membatalkan pesanan: "+b.message,"error")}}}),document.addEventListener("click",async i=>{if(i.target.closest(".complete-order")){const c=i.target.closest(".complete-order"),s=c.dataset.orderId,h=c.dataset.takenDate,b=JSON.parse(c.dataset.items||"[]");try{for(const u of b){const{error:v}=await y.rpc("adjust_reserved_stock",{p_variant_id:u.variantId,p_adjustment:-u.qty});if(v)throw v;await X({variantId:u.variantId,type:"penjualan",id:s,quantity:u.qty,price:u.price,date:h})}const{error:d}=await y.from("pesanan_penjualan").update({status_pesanan:"selesai"}).eq("id",s);if(d)throw d;m("Pesanan berhasil diselesaikan","success"),await B()}catch(d){console.error("Order processing failed:",d),m("Gagal menyelesaikan pesanan: "+d.message,"error")}}}),_e();const r=document.createElement("script");r.src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js",document.body.appendChild(r);const o=document.createElement("script");o.src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js",document.body.appendChild(o);async function l(){B(),ye(),t&&t.addEventListener("click",()=>z("add")),a&&a.addEventListener("click",B),n&&n.addEventListener("click",()=>{H(),O()}),await Z()}});window.deleteSale=ue;window.showSaleDetail=function(e){console.log("Showing details for sale:",e)};

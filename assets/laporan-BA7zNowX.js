import{s as k}from"./db_conn-CuK1RHLN.js";import{l as rt,c as st,d as ot}from"./import-C1NTNbVL.js";rt();const{jsPDF:pt}=window.jspdf;(async()=>await st())();function I(m,x="success"){const P=document.getElementById("toastContainer"),S=document.createElement("div");S.classList.add("toast"),S.setAttribute("role","alert"),S.setAttribute("aria-live","assertive"),S.setAttribute("aria-atomic","true"),S.classList.add(x==="success"?"bg-success":"bg-danger"),S.classList.add("text-white");const $=document.createElement("div");$.classList.add("toast-body"),$.textContent=m,S.appendChild($),P.appendChild(S),new bootstrap.Toast(S,{autohide:!0,delay:3e3}).show(),S.addEventListener("hidden.bs.toast",()=>{S.remove()})}function it(){const m=document.querySelector(".report-container");m&&m.scrollTo({top:0,behavior:"smooth"})}function Y(m){return Array.isArray(m)?m.length===0:typeof m=="object"&&m!==null?m.logs?m.logs.length===0:Object.keys(m).length===0:!1}document.addEventListener("DOMContentLoaded",async()=>{await ot();const m=document.getElementById("jenis"),x=document.getElementById("startDate"),P=document.getElementById("endDate"),S=document.getElementById("generateBtn"),$=document.getElementById("downloadBtn"),A=document.getElementById("report-content"),N=document.getElementById("generateText"),q=document.getElementById("generateSpinner"),T=document.getElementById("groupByContainer"),B=document.createElement("label");B.className="form-label",B.innerHTML="Pengelompokkan",T.appendChild(B);const v=document.createElement("select");v.id="groupBy",v.className="form-select",v.innerHTML="",T.style.display="none",m.addEventListener("change",()=>{m.value==="Pembelian"||m.value==="Penjualan"?(T.style.display="block",Q(m.value),T.appendChild(v)):T.style.display="none"});function Q(e){e==="Pembelian"?v.innerHTML=`

import"./modulepreload-polyfill-B5Qt9EMX.js";import{s as g}from"./db_conn-C7Nb5uSA.js";import{p as D}from"./import-Cn2uwdsk.js";let L;function m(e,t="success"){const a=document.getElementById("toastContainer");if(!a){console.error("Toast container not found!");return}const n=document.createElement("div");n.className=`toast align-items-center text-white bg-${t==="success"?"success":"danger"} border-0`,n.setAttribute("role","alert"),n.setAttribute("aria-live","assertive"),n.setAttribute("aria-atomic","true"),n.innerHTML=`
    ...
    `,a.appendChild(n),new bootstrap.Toast(n,{autohide:!0,delay:3e3}).show(),n.addEventListener("hidden.bs.toast",()=>{n.remove()})}function T(e){switch(e){case"sesuai":return"bg-success";case"tidak_sesuai":return"bg-warning";case"disesuaikan":return"bg-info";default:return"bg-secondary"}}async function q(e="add",t=null){const a=document.getElementById("stockOpnameModal");a.dataset.mode=e,document.getElementById("tanggal").valueAsDate=new Date,document.querySelector("#productsTable tbody").innerHTML="";const n=a.querySelector(".modal-title"),s=document.getElementById("submitLogButton");e==="edit"?(n.textContent="Edit Stock Opname",s.textContent="Update",a.dataset.logId=t.id,a.dataset.logData=JSON.stringify(t),document.getElementById("tanggal").valueAsDate=new Date(t.tanggal),await _(!0,t)):(n.textContent="Buat Stock Opname Baru",s.textContent="Simpan",await _(!1))}async function _(e=!1,t=null,a=1,n=10){const s=document.querySelector("#stockOpnameModal table tbody"),o=document.getElementById("selectAll");try{s.innerHTML=`
            ...
            </tr>`}}async function P(){document.getElementById("stockOpnameModal").dataset.mode==="edit"?await N():await j()}async function j(){const e=document.getElementById("tanggal"),t=document.getElementById("submitLogButton"),a=document.querySelectorAll("#stockOpnameModal .product-checkbox:checked");if(!e.value){m("Tanggal harus diisi","error");return}if(a.length===0){m("Pilih minimal 1 produk","error");return}try{t.disabled=!0,t.innerHTML='<span class="spinner-border spinner-border-sm" role="status"></span> Menyimpan...';const{data:n,error:s}=await g.from("log_opname").insert({tanggal:e.value,status_log:"draft",waktu_dibuat:new Date().toISOString()}).select("id").single();if(s)throw s;const o=Array.from(a).map(r=>({id_log:n.id,id_varian:parseInt(r.dataset.variantId),stok:null,status_item_log:"pending"})),{error:i}=await g.from("item_opname").insert(o);if(i)throw i;m("Stock opname berhasil dibuat"),L.hide(),await I()}catch(n){console.error("Error submitting stock opname:",n),m("Gagal menyimpan stock opname: "+n.message,"error")}finally{t&&(t.disabled=!1,t.innerHTML="Simpan")}}async function N(){const e=document.getElementById("stockOpnameModal"),t=document.getElementById("tanggal"),a=document.getElementById("submitLogButton"),n=document.querySelectorAll("#stockOpnameModal .product-checkbox:checked"),s=e.dataset.logId;if(!t.value){m("Tanggal harus diisi","error");return}try{a.disabled=!0,a.innerHTML='<span class="spinner-border spinner-border-sm" role="status"></span> Menyimpan...';const{error:o}=await g.from("log_opname").update({tanggal:t.value}).eq("id",s);if(o)throw o;const{data:i,error:r}=await g.from("item_opname").select("id_varian").eq("id_log",s);if(r)throw r;const p=i.map(h=>h.id_varian),u=Array.from(n).map(h=>parseInt(h.dataset.variantId)).filter(h=>!p.includes(h));if(u.length>0){const h=u.map(d=>({id_log:s,id_varian:d,stok:null,status_item_log:"pending"})),{error:c}=await g.from("item_opname").insert(h);if(c)throw c}m("Perubahan stock opname berhasil disimpan"),L.hide(),await I()}catch(o){console.error("Error editing stock opname:",o),m("Gagal menyimpan perubahan: "+o.message,"error")}finally{a&&(a.disabled=!1,a.innerHTML="Update")}}async function G(){try{const{error:e}=await g.from("log_opname").delete().eq("status_log","draft");if(e)throw e;m("Log stock opname berhasil dihapus"),await I()}catch(e){console.error("Error deleting stock opname:",e),m("Gagal menghapus log stock opname: "+e.message,"error")}}async function I(){try{const e=await V();Q(e),e?await H():z()}catch(e){console.error("Error in checkActiveLog:",e)}}async function V(){try{const{data:e,error:t}=await g.from("log_opname").select("id").eq("status_log","draft").maybeSingle();if(t)throw t;return!!e}catch(e){return console.error("Error checking active draft log:",e),!1}}function z(){const e=document.getElementById("active");e.innerHTML=`
        <div class="alert alert-info">
            Tidak ada log aktif saat ini. Klik tombol "+ Buat log baru" untuk memulai stock opname.
        </div>`}async function H(){const e=document.getElementById("active");e.innerHTML=`
        <div class="card">
            <div class="card-body">
            ...
            </div>`,document.getElementById("editLogBtn").addEventListener("click",()=>{q("edit",t),L.show()}),document.getElementById("deleteLogBtn").addEventListener("click",async()=>{confirm("Apakah Anda yakin ingin menghapus log ini?")&&await G()}),R(t.items,t.id),U(t.id,n)}catch(t){console.error("Error loading active log:",t),e.innerHTML=`
            <div class="alert alert-danger">
                Gagal memuat log aktif. Silakan coba lagi.
            </div>`}}function R(e,t){const a=document.getElementById("activeLogItems");a.innerHTML="",e.forEach(n=>{const s=n.stok!==null?n.stok:"",o=n.produk_varian.jumlah_stok||0,i=o===s?"sesuai":n.status_item_log,r=document.createElement("tr");r.innerHTML=`
        ...
        `,a.appendChild(r)})}function F(){document.addEventListener("input",async function(e){if(e.target.classList.contains("physical-stock")){const t=e.target,a=t.dataset.itemId,n=t.closest("tr"),s=parseInt(n.querySelector("td:nth-child(3)").textContent)||0,o=parseInt(t.value)||0,i=o-s,r=n.querySelector(".badge"),p=n.querySelector(".btn-match"),u=n.querySelector(".difference-preview");u.textContent=`(${i>0?"+":""}${i})`,u.className=`difference-preview small ms-2 ${i===0?"text-success":"text-warning"}`,i===0?(r.className="badge bg-success",r.textContent="sesuai",p.disabled=!0,await B(a,"sesuai",o)):(r.className="badge bg-warning",r.textContent="tidak_sesuai",p.disabled=!1,await B(a,"tidak_sesuai",o)),await C()}})}function J(){document.addEventListener("click",async function(e){if(e.target.classList.contains("btn-match")){const t=e.target,a=t.dataset.itemId,n=t.closest("tr"),s=n.querySelector(".physical-stock"),o=n.querySelector("td:nth-child(2)").dataset.variantId,i=parseInt(n.querySelector("td:nth-child(3)").textContent)||0,r=parseInt(s.value)||0,p=r-i;if(console.log("selisih: ",p),p===0){m("Tidak ada penyesuaian diperlukan");return}try{t.disabled=!0,t.innerHTML='<span class="spinner-border spinner-border-sm" role="status"></span>';const u=p>0?"penyesuaian_masuk":"penyesuaian_keluar";await D({variantId:o,type:u,id:a,quantity:Math.abs(p),price:0,date:new Date().toISOString()}),await B(a,"disesuaikan",r,new Date().toISOString());const h=n.querySelector(".badge");h.className="badge bg-info",h.textContent="disesuaikan",n.querySelector("td:nth-child(3)").textContent=r,m(`Stok berhasil disesuaikan (${p>0?"+":""}${p})`),await C()}catch(u){m("Gagal menyamakan stok: "+u.message,"error")}finally{t.disabled=!1,t.textContent="Match"}}})}function W(){document.addEventListener("click",async function(e){if(e.target.classList.contains("btn-remove-item")){const t=e.target,a=t.dataset.itemId;if(t.dataset.logId,confirm("Apakah Anda yakin ingin menghapus item ini dari log?"))try{t.disabled=!0,t.innerHTML='<span class="spinner-border spinner-border-sm" role="status"></span>';const{error:n}=await g.from("item_opname").delete().eq("id",a);if(n)throw n;m("Item berhasil dihapus"),await H()}catch(n){m("Gagal menghapus item: "+n.message,"error"),t.disabled=!1,t.innerHTML="✖"}}})}async function B(e,t,a,n=null){const s={status_item_log:t,stok:a};n&&(s.waktu_penyesuaian=n);const{error:o}=await g.from("item_opname").update(s).eq("id",e);if(o)throw o}async function A(e){const{data:t,error:a}=await g.from("item_opname").select("status_item_log").eq("id_log",e).in("status_item_log",["pending","tidak_sesuai"]);if(a)throw a;return t.length>0}async function C(){const e=document.getElementById("finalizeBtn");if(!e)return;const t=e.dataset.logId;if(!t)return;const a=await A(t);e.disabled=a,e.title=a?"Masih ada item yang belum disesuaikan":""}function U(e,t){const a=document.getElementById("finalizeBtn");a&&(a.dataset.logId=e,a.disabled=t,a.title=t?"Masih ada item yang belum disesuaikan":"",t||a.addEventListener("click",async()=>{confirm("Apakah Anda yakin ingin memfinalisasi stock opname ini?")&&await K(e)}))}async function K(e){try{const{error:t}=await g.from("log_opname").update({status_log:"final",waktu_penyelesaian:new Date().toISOString()}).eq("id",e);if(t)throw t;m("Stock opname berhasil difinalisasi"),await I()}catch(t){m("Gagal memfinalisasi: "+t.message,"error")}}function Q(e){const t=document.querySelector('[data-bs-target="#stockOpnameModal"]');t&&(t.disabled=e,t.title=e?"Selesaikan log aktif terlebih dahulu":"")}async function $(){const e=document.getElementById("history");e.innerHTML=`
        <div class="text-center my-4">
            <div class="spinner-border" role="status">
                <span class="visually-hidden">Loading...</span>
            </div>
            ...
            `,n.appendChild(o)}),document.querySelectorAll(".btn-view-details").forEach(s=>{s.addEventListener("click",function(){const o=this.dataset.logId,i=t.find(r=>r.id==o);X(i)})})}catch(t){console.error("Error loading riwayat data:",t),e.innerHTML=`
            <div class="alert alert-danger">
                Gagal memuat riwayat stock opname: ${t.message}
            </div>`}}function X(e){let t=document.getElementById("riwayatDetailsModal");t||(t=document.createElement("div"),t.className="modal fade",t.id="riwayatDetailsModal",t.innerHTML=`

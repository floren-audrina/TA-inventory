import{s}from"./db_conn-CuK1RHLN.js";import{n as h}from"./navbar-BXvx_Lss.js";import{c as v,d as w,p as E}from"./import-wcpIJIsu.js";fetch("navbar.html").then(t=>t.text()).then(t=>{document.getElementById("navbar").innerHTML=t;const a=document.createElement("script");a.type="module",a.src=h,document.body.appendChild(a)}).catch(t=>console.error("Error loading navbar:",t));(async()=>(await v(),initPageSpecificFunctions()))();function y(t){return t?new Date(t).toLocaleDateString("id-ID",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!1,timeZone:"Asia/Jakarta"}):"-"}function l(t,a="success"){const e=document.getElementById("toastContainer"),n=document.createElement("div");n.classList.add("toast"),n.setAttribute("role","alert"),n.setAttribute("aria-live","assertive"),n.setAttribute("aria-atomic","true"),n.classList.add(a==="success"?"bg-success":"bg-danger"),n.classList.add("text-white");const r=document.createElement("div");r.classList.add("toast-body"),r.textContent=t,n.appendChild(r),e.appendChild(n),new bootstrap.Toast(n,{autohide:!0,delay:3e3}).show(),n.addEventListener("hidden.bs.toast",()=>{n.remove()})}function c(){return new URLSearchParams(window.location.search).get("variantId")}async function b(t){const{data:a,error:e}=await s.from("produk_varian").select(`
        ...
        `).eq("id",t).single();return e?(console.error("Error fetching variant details:",e),null):a}async function T(){const t=document.getElementById("tipe_riwayat");t.innerHTML='<option value="">Memuat tipe riwayat...</option>';try{const{data:a,error:e}=await s.rpc("get_enum_values",{enum_name:"tipe_riwayat"});if(e)throw e;t.innerHTML='<option value="" disabled selected>Pilih tipe riwayat</option>',a.filter(r=>r.value==="penyesuaian_masuk"||r.value==="penyesuaian_keluar").forEach(r=>{const d=document.createElement("option");d.value=r.value,d.text=r.value.charAt(0).toUpperCase()+r.value.slice(1).replace("_"," "),t.appendChild(d)})}catch(a){console.error("Error fetching tipe_riwayat enum:",a),t.innerHTML='<option value="">Error loading tipe riwayat</option>',l("Gagal memuat tipe riwayat","error")}}async function k(t){t.preventDefault();const a=c();document.getElementById("tanggal").value;const e=document.getElementById("tipe_riwayat").value,n=parseInt(document.getElementById("qty").value,10);if(document.getElementById("harga")&&parseFloat(document.getElementById("harga").value),!a||!e||!n){alert("Harap isi semua field yang diperlukan.");return}await E({variantId:a,quantity:n})}async function g(t){const a=document.getElementById("historyTableBody");try{if(a.innerHTML=`
            ...
            </tr>`,l("Gagal memuat riwayat stok","error")}}const i={type:null,tipe:null};function L(){var a,e;const t=document.getElementById("filterTipe");t&&t.addEventListener("click",function(n){if(this.checked&&i.type==="tipe"){this.checked=!1,u();return}i.type="tipe",I()}),(a=document.getElementById("applyFilter"))==null||a.addEventListener("click",f),(e=document.getElementById("resetFilter"))==null||e.addEventListener("click",u)}async function I(){var a,e;const t=document.getElementById("filterOptionsContainer");t.innerHTML="",document.getElementById("filterCard").classList.remove("d-none"),t.innerHTML=`
        <div class="mb-3">
        ...
        <tr>
        ...
        </tr>`;try{let e=s.from("riwayat_stok").select("*").eq("id_varian",a).order("tanggal",{ascending:!1});i.type==="tipe"&&i.tipe&&(e=e.eq("tipe_riwayat",i.tipe));const{data:n,error:r}=await e;if(r)throw r;M(n)}catch(e){console.error("Error filtering entries:",e),l("Gagal memfilter entri","error"),t.innerHTML=`
            <tr>
                <td colspan="8" class="text-center text-danger">
                    Gagal memuat entri riwayat
                </td>
            </tr>`}}function M(t){const a=document.getElementById("historyTableBody");if(a.innerHTML="",!t||t.length===0){a.innerHTML=`
            <tr>
            ...
            <td>${e.stok_sisa??"-"}</td>
        `,a.appendChild(n)})}document.addEventListener("DOMContentLoaded",async()=>{await w();const t=c();if(!t){alert("ID varian tidak valid. Kembali ke halaman produk."),window.location.href="product.html";return}[].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]')).map(function(r){return new bootstrap.Tooltip(r)}),await T(),await g(t),L();const e=document.getElementById("historyForm");e&&e.addEventListener("submit",k);const n=document.getElementById("backButton");n&&n.addEventListener("click",()=>{window.location.href="product.html"})});

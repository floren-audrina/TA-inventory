import"./modulepreload-polyfill.js";import{n as $,d as V}from"./import.js";import{s as d}from"./db_conn.js";import{i as G,c as A}from"./auth.js";fetch("navbar.html").then(e=>e.text()).then(e=>{document.getElementById("navbar").innerHTML=e;const t=document.createElement("script");t.type="module",t.src=$,document.body.appendChild(t)}).catch(e=>console.error("Error loading navbar:",e));G();(async()=>await A())();let T,B=null,o={type:null,category:null,subcategory:null,supplier:null,variant:null};const j=new Map([[.25,"1/4 lusin"],[.5,"1/2 lusin"],[1,"1 lusin"]]);function R(){var e,t;document.querySelectorAll('input[name="filterGroup"]').forEach(a=>{a.addEventListener("click",function(r){if(this.checked&&o.type===this.value){this.checked=!1,I();return}o.type=this.value,K()})}),(e=document.getElementById("applyFilter"))==null||e.addEventListener("click",H),(t=document.getElementById("resetFilter"))==null||t.addEventListener("click",I)}async function K(){var t,a;const e=document.getElementById("filterOptionsContainer");switch(e.innerHTML="",document.getElementById("filterCard").classList.remove("d-none"),o.type){case"kategori":e.innerHTML=`
                <div class="mb-3">
        ...
        `}function U(){document.getElementById("searchInput").value="",L(),q()}async function x(e="add",t=null){B=t,document.getElementById("productModal"),document.getElementById("productForm").reset(),document.getElementById("variantsTableBody").innerHTML="",document.getElementById("productModalLabel").textContent=e==="add"?"Tambah Produk":"Edit Produk",document.getElementById("saveProductBtn").textContent=e==="add"?"Simpan":"Update",e==="edit"&&t?await z(t):w(),T.show()}async function z(e){try{const{data:t,error:a}=await d.from("produk").select(`
                id, 
                nama, 
                id_subkategori, 
                id_supplier,
    ...
    `,a.appendChild(r),new bootstrap.Toast(r,{autohide:!0,delay:3e3}).show(),r.addEventListener("hidden.bs.toast",()=>{r.remove()})}function oe(e){window.location.href=`history.html?variantId=${e}`}document.addEventListener("DOMContentLoaded",async()=>{T=new bootstrap.Modal(document.getElementById("productModal")),R(),await V(),document.getElementById("addProductBtn").addEventListener("click",()=>x("add")),document.getElementById("addVariantBtn").addEventListener("click",()=>w()),document.getElementById("saveProductBtn").addEventListener("click",J);const e=document.getElementById("searchInput");document.getElementById("searchButton").addEventListener("click",()=>{e.value.trim()&&document.getElementById("searchIcon").innerHTML.includes("bi-x")?U():_()}),e.addEventListener("keyup",a=>{L(),a.key==="Enter"&&_()}),e.addEventListener("input",L),await Promise.all([Y(),ee(),re(),E()]),document.getElementById("variantsTableBody").addEventListener("click",a=>{a.target.classList.contains("remove-variant")&&a.target.closest("tr").remove()})});window.editProduct=async function(e){await x("edit",e)};window.deleteProduct=ne;window.viewVariantHistory=oe;window.deleteVariant=ae;
